...
- Matched baseline route: `grud_h1_baseline_c4_20260717_v1` supplies 118 registered H1 channels over the exact 50,350 frozen anchors and patient split, while the unchanged r9 sidecar supplies the same six-M4, 29-process, 414-factor target. `grud_h1_joint_m4_v2` uses a 1,596,987-parameter GRU-D history encoder and a plain causal GRUCell target adapter. It contains no multi-resolution encoder, Transformer, relation path, target attention, or temporal fusion. The first P100 run was cancelled after logging step 2900; its hash-verified step-2500 model, AdamW, scheduler, and GradScaler state are the recovery authority. The separate resume route continues the same global schedule from 2500 to 4000 and declares the unavoidable RNG reset from the v1 checkpoint.
...
The active hosted delivery surface is the published private `vanila111/trauma-predict-relation-v2-p100-r9` Notebook. Its private bundle must already be attached under Input before Save & Run All; a raw `.ipynb` import requires Add Input first. With P100 selected and Internet off, it verifies the single runtime archive plus all 28 logical wheel hashes, installs the frozen cu126 runtime under `/kaggle/temp`, proves a real `sm_60` CUDA backward in a child process, and then delegates to `run_relation_v2_p100_bundle.py`.

The recovered matched-baseline delivery surface is `notebooks/kaggle/trauma_predict_grud_h1_joint_m4_v2_resume_2500.ipynb`. It binds the original private GRU-D science Dataset, the verified P100 runtime Dataset, and a small private resume Dataset containing the cancelled v4 step-2500 checkpoint and evidence. It restores the original AdamW, scheduler, and GradScaler states; reconstructs sampler epoch 52 after eight consumed microbatches; and runs exactly 1,500 new optimizer updates to global step 4000. The old checkpoint did not contain RNG state, so this first continuation is reproducible but not bitwise identical to an uninterrupted run. New checkpoints archive the RNG, sampler, trainer cursor, and best-state metadata required to build and validate a later exact-continuation route; that future route is not included in this release. Full 6,309-anchor teacher-forced evaluation and free-running evaluation remain separate jobs.

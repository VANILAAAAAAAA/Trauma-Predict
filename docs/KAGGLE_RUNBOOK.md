...
The active formal target is the structured `multires_event_v2_m4_relation_v2` route on one P100. Open the published private `vanila111/trauma-predict-relation-v2-p100-r9` Notebook, confirm that `vanila111/trauma-predict-relation-v2-p100-r9-bundle` is listed under Input, select P100, keep Internet off, and choose Save & Run All. A raw `.ipynb` import must use Add Input before running. Bundle schema v3 stores the 28 hash-locked Python 3.12 `torch 2.10.0+cu126` wheels inside one safely named `p100_torch_2_10_cu126_cp312_wheelhouse.blob`; Kaggle therefore never needs to preserve an individual `+cu126` filename. The Notebook verifies the archive and every regular top-level member, extracts and installs under `/kaggle/temp`, removes the temporary wheel extraction, and rejects any runtime without `sm_60` before optimizer steps 250, 1500, 2750, and 4000. Later runs reuse the persisted final teacher evaluation and resume hash-bound free-running chunks until all 6,309 validation anchors have 100 trajectories.

The expected bootstrap sequence is `RELATION_V2_P100_BOUND_INPUT_OK`, `RELATION_V2_P100_RUNTIME_ARCHIVE_OK`, `RELATION_V2_P100_RUNTIME_WHEELS_OK files=28`, `RELATION_V2_P100_RUNTIME_INSTALLED`, and `RELATION_V2_P100_CU126_CUDA_BACKWARD_OK`. Failure before the final marker means training has not started.
...
git checkout --detach stage-a-hour-modernbert-4000-20260709
...
Linking a Kaggle Notebook to GitHub is optional. For this project, cloning a pinned tag is more reproducible than relying on notebook sync state. Use `stage-a-hour-modernbert-4000-20260709` for the preferred-encoder Stage A run after that tag is pushed.

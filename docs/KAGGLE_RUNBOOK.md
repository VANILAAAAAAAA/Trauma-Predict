...
The active formal target is the structured `multires_event_v2_m4_relation_v2` route on one P100. Open the published private `vanila111/trauma-predict-relation-v2-p100-r9` Notebook, confirm that `vanila111/trauma-predict-relation-v2-p100-r9-bundle` is listed under Input, select P100, keep Internet off, and choose Save & Run All. A raw `.ipynb` import must use Add Input before running. The Notebook verifies the clean source, frozen data identities, and complete hash-locked Python 3.12 `torch 2.10.0+cu126` wheelhouse; it rejects any runtime without `sm_60` before advancing through optimizer steps 250, 1500, 2750, and 4000 across successive Save Runs. Later runs reuse the persisted final teacher evaluation and resume hash-bound free-running chunks until all 6,309 validation anchors have 100 trajectories.

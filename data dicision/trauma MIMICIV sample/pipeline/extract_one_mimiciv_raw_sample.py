...
def extract_one_hadm(cfg: dict, hadm_id: str | None):
    ...
    def ed_src(table_key: str) -> str:
        return os.path.join(cfg["ed_root"], cfg["ed_tables"][table_key])
    ...
    # ED linkage and raw ED rows for G2 pre-ICU fields.
    ed_rows = []
    ed_stay_ids = set()
    ed_linkage_csv = cfg.get("ed_linkage_csv")
    if ed_linkage_csv:
        with gz_csv(ed_linkage_csv) as f:
            reader = csv.DictReader(f)
            for row in reader:
                if row.get("hadm_id") == hadm_id:
                    ed_rows.append(row)
                    if row.get("stay_id"):
                        ed_stay_ids.add(row["stay_id"])
        write_rows(os.path.join(out_dir, "05_ed_linkage_raw.csv"), ed_rows)
        report["tables"]["ed_linkage"] = {
            "source": ed_linkage_csv,
            "output": os.path.join(out_dir, "05_ed_linkage_raw.csv"),
            "written_rows": len(ed_rows),
            "ed_stay_ids": sorted(ed_stay_ids),
        }
    if ed_stay_ids:
        report["tables"]["edstays"] = filter_table_by_keys(ed_src("edstays"), os.path.join(out_dir, "06_edstays_raw.csv"), {"stay_id": ed_stay_ids})
        report["tables"]["ed_triage"] = filter_table_by_keys(ed_src("triage"), os.path.join(out_dir, "07_ed_triage_raw.csv"), {"stay_id": ed_stay_ids})
        report["tables"]["ed_vitalsign"] = filter_table_by_keys(ed_src("vitalsign"), os.path.join(out_dir, "08_ed_vitalsign_raw.csv"), {"stay_id": ed_stay_ids})
        report["tables"]["ed_diagnosis"] = filter_table_by_keys(ed_src("diagnosis"), os.path.join(out_dir, "09_ed_diagnosis_raw.csv"), {"stay_id": ed_stay_ids})

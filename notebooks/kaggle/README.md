...
Stage A.1 residual warm-start uses `train_stage_a1_residual.ipynb`, `run_stage_a1_residual.py`, `configs/train/t4x2_stage_a1_residual_smoke.yaml`, and `configs/train/t4x2_stage_a1_residual.yaml`. It requires a mounted Stage A checkpoint Dataset and trains `ModernBERT-base + field-aware HOUR adapter + H0 residual NEXT_HOUR values-only`.
